...
---
## ADR-19: Endpoint data view and live TUI freshness (2026-08-01)
- **Status:** accepted
- **Context:** The dashboard exposed flows and analysis tables but did not provide an endpoint-centric view of actual response data. On Windows, live capture failures or SQLite startup/contention errors could leave the dashboard looking empty; on Mac, repeated full refreshes made analysis appear delayed. A response cache would risk masking rows written during capture.
- **Decision:** Add a Data tab over body-bearing normalized endpoints. Select the newest body-bearing flow per endpoint and render method, endpoint, type, Content-Encoding, size, status, response-header names, and a clipped safe preview. Detect JSON/HTML/XML/text plus gzip/ZIP/binary using both stored MIME/header metadata and magic bytes; recognize base64-wrapped binary bodies without mutating catalog data. Persist capture and analysis status/error metadata and guard startup, refresh, final-analysis retry, and worker teardown paths so failures are visible rather than silently producing empty views. Keep schema inference→Rosetta chained, with sensitive scanning concurrent beside it; SNI remains outside the core pool. Do not add a response cache: use WAL and active-tab-only refresh during capture, then full reload on completion/tab activation.
- **Consequences:** Live rows remain fresh and endpoint data is useful for both text and binary responses. The UI does more defensive lifecycle handling and a final analysis retry, but a real Windows Playwright run is still required. A pathological browser/network hang can still make graceful shutdown wait; a bounded fallback remains a follow-up.

---
## ADR-20: Continuous real-browser capture with TUI-controlled detach (2026-08-01)
- **Status:** accepted (implemented Session 30)
- **Context:** The existing browse driver could attach to a user's Chromium browser, but the primary user-facing request was to run continuous capture while the user browses normally, including from the TUI. Textual workers do not receive terminal `KeyboardInterrupt`, so a TUI stop needed an explicit cross-thread signal. The user's browser must never be closed when Glyph only attached to it.
- **Decision:**
  1. `--browser` is the primary user-facing spelling for continuous real-browser capture on `glyph run live` and `glyph capture live`; `--browse` remains backward compatible. CDP attach to the user's Chrome/Edge/Brave endpoint is primary, with a real-browser launch fallback selected by an optional browser name.
  2. A URL scopes capture to a fresh target tab plus popups. Omitting the URL explicitly enables all-tabs capture. Page identity de-duplication prevents context/page and popup lineage events from recording the same tab twice.
  3. The TUI home screen exposes a browser-live checkbox. The dashboard passes a `threading.Event` to the Playwright worker; `s` / Stop capture and graceful app shutdown set it. The worker performs Playwright cleanup on its own thread: CDP attach detaches and preserves the user's browser, while Glyph-owned launch fallback closes its context/browser.
  4. Browser/context close and disconnect events terminate capture, persist `capture_status=done`, and allow the normal schema → Rosetta → sensitive → SNI finalization lifecycle to proceed.
- **Consequences:** Real browser credentials, tabs, and extensions remain available in CDP attach mode, but the user must start Chromium with remote debugging enabled. All-tabs mode is intentionally explicit because it can capture unrelated sensitive tabs. TUI stop is graceful rather than forceful; a pathological browser/network hang can still require a future bounded shutdown fallback. The real Playwright workflow remains an on-device Windows verification item.

---
## ADR-21: Cross-platform quit confirmation and coordinated SNI lane (2026-08-01)
- **Status:** accepted (implemented Session 31)
- **Context:** Textual's native Ctrl+Q path called `exit()` directly, bypassing Glyph's quit modal on macOS. The confirmation modal dismissed before its callback on some backends, making Windows confirmation appear inert while workers continued. SNI had a separate target-pinned lifecycle but was still awaited only after core analysis, so slow network recon did not overlap the parallel core lanes.
- **Decision:** Route native Ctrl+Q and Ctrl+C through the same confirmation screen as q/Escape; invoke shutdown directly from the confirmed modal button; wait asynchronously on the Textual event loop for tracked workers; and use a visible 10-second timeout as an escape hatch for pathological workers. Add `run_pipeline()` to submit core analysis and the independent target-pinned SNI lifecycle concurrently, while retaining serial per-candidate SNI network work to respect bounded public lookup behavior.
- **Consequences:** macOS and Windows use one confirmation path, and graceful shutdown remains SQLite/Playwright-safe until the timeout. A timeout can leave a Python worker alive briefly because Python threads are not forcibly killed. SNI overlaps core analysis but does not fan out individual public-network candidate lookups. ADR-15's statement that SNI remains outside the core pool is retained as a lifecycle boundary; this ADR adds coordinated overlap around that boundary.

---
## ADR-22: Pumped real-browser capture lifecycle (2026-08-01)
- **Status:** accepted (implemented Session 32)
- **Context:** The synchronous Playwright browse worker waited on a Python event without pumping Playwright, so user browsing could produce no visible flows and browser-close events were delayed. Launch fallback profiles could restore stale tabs, and users lacked consistent executable/profile configuration.
- **Decision:** Pump sync Playwright through the active page or context while browsing; persist and render an explicit stop reason; close stale pages only for Glyph-owned launch fallbacks; keep CDP-attached user tabs untouched; support explicit and environment-configured executable/profile paths; and clear the active target for all-tabs capture so its new rows use the reserved unassigned bucket.
- **Consequences:** Live request/response and close events continue dispatching, and the TUI/CLI explain browser closure. A CDP-attached external browser's existing command-line flags cannot be changed by Glyph; root Linux remains an unavoidable sandbox limitation. All-tabs rows use target 0 and can coexist with historical catalog rows; strict capture-session isolation remains a future schema feature. Windows on-device verification remains recommended.

---
## ADR-23: Geo-block recovery and explicit browser targeting (2026-08-01)
- **Status:** accepted (implemented Session 33)
- **Context:** Users needed explicit `-t/--target` and `-b/--browser` forms, configurable browser executable/profile paths, a useful geo-block response, and reliable quit/navigation behavior across macOS and Windows. A direct `glyph run live` dashboard exposed a Textual root-screen callback underflow when geo recovery used `switch_screen`.
- **Decision:** Normalize positional and explicit target forms to HTTPS URLs; treat browser/path/profile/proxy settings as browse-mode configuration and force launch when an explicit configuration must not attach to an unrelated CDP process; classify geo-blocks conservatively using HTTP 451, explicit country/region language, or strong network-denial signals; persist geo metadata immediately; and return to the Home settings surface through a deferred modal callback. Pop a dashboard only when Home is already below it; for a direct root dashboard, push the inert stopped dashboard's Home screen rather than calling `switch_screen`.
- **Consequences:** `glyph run live --browser brave --target google.com` and `glyph run live -b brave -t google.com` are supported, while ordinary 403/WAF responses do not automatically trigger geo UX. Browser path and profile settings are explicit and portable across Windows/macOS. The TUI has one authoritative settings surface. External Ctrl+C outside the confirmation flow remains an on-device cleanup verification item; Windows/CDP process-boundary verification remains open.
