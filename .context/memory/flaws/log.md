...
- **Flaw:** The protocol's `context-sync verify` computes SHA256 hashes against on-disk file bytes, but on Windows with `core.autocrlf=true`, `.context/core/*` files are checked out with CRLF line endings while `MANIFEST.sha256` contains hashes computed against LF-only blob content. This causes `verify` to fail on every core file on Windows, making the integrity check unusable on that platform.
- **Symptom:** Session 11 on Windows: `pwsh -File .context/core/bin/context-sync.ps1 verify` reported `FAILED` for all 20 core files. `context-sync rollback` was suggested as the fix, but rollback only restores LF blobs from git history — the next checkout reintroduces CRLF, so verify will fail again.
- **Root cause:** Windows git's `core.autocrlf=true` converts LF blobs to CRLF on checkout. The PowerShell port (`context-sync.ps1`) hashes the on-disk bytes via `Get-FileHash`, which sees CRLF, not the LF-only blob the manifest was computed against. The sh script on macOS/Linux doesn't hit this because those platforms default to preserving LF.
- **Suggested fix:** Two concrete changes to the package:
