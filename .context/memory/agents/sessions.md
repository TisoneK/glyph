...
- **Agent:** Buffy | **Model:** deepseek/deepseek-v4-flash (system prompt states the exact model) | **Platform:** bao@local macOS (Darwin 24.6.0; account name `bao`, user is **Tisone Kironget**) | **Role:** engineer | **Core:** 0.5.0
- **Task:** Target (user follow-up): now that Chromium is installed, run `glyph run live https://example.com` and verify the live Textual dashboard streams the capture in real time (worker + progress + ✓ captured header) on-device.
- **Commits:** none (verification session — no product code changed). This `chore(context):` bookkeeping.
- **Outcome:** done + VERIFIED — the longest-standing live-path open item (real live-TUI verification, carried since Session 15) is closed. tmux is NOT installed on this Mac, so I drove the full-screen Textual dashboard through a real pty with a small Python harness (`.venv/bin/glyph run live https://example.com --db /tmp/glyph-tui.db`, TERM=xterm-256color, 120x30): `● LIVE 00:00` header seen at t=1.6s, FLOWS streamed 0→2→22→23→46 across the 1s ticks (worker + WAL writes visible in real time), header flipped to `✓ captured · 00:14` at t=14.8s, `q` quit cleanly at t=22.8s. No `✗ failed`, no tracebacks. Catalog confirms on-device persistence: 46 flows, 1 page, target example.com (`glyph target show`). The 1s/4s tick + guarded analysis + WAL concurrent-writer design works on-device exactly as the mock tests predicted.
- **Open items:** the Windows-box live-TUI verification (playwright present there) remains the only unverified live path — otherwise the backlog is unchanged (TUI target picker, DuckDB backend, Daraja recipe, 3.13 retarget).
- **Report:** .context/memory/reviews/2026-08-01-live-tui-on-device.md

---
## 2026-08-01 — Session 24
- **Agent:** Buffy | **Model:** deepseek/deepseek-v4-flash (system prompt states the exact model) | **Platform:** bao@local macOS (Darwin 24.6.0; account name `bao`, user is **Tisone Kironget**) | **Role:** engineer | **Core:** 0.5.0
- **Task:** Target (user chat + ask_user clarification): 'When capturing live why shouldn't the modules run simultaneously? I think they are waiting for each other. Target: parallel capturing from different tools from the same target.' Clarified: the ANALYSIS stages (sni, rosetta, sensitive, 'and all') should run in parallel.
- **Commits:** 1 product commit (`cabce93` feat(pipeline)) + this `chore(context):` bookkeeping. See `git log`.
- **Outcome:** done — the post-capture stages ran strictly sequentially (schema, rosetta, sensitive, then snihunt) even though only schema→rosetta actually depends (rosetta's dom_attribute reads schema's enum-candidate fields). New `glyph/pipeline.py::run_analysis()` runs THREE LANES concurrently over a ThreadPoolExecutor: schema→rosetta (chained), sensitive, snihunt — the slowest stage (bounded network recon) no longer waits behind the others. Opt-out flags skip lanes; result shape {sch, ros, sens, sni} unchanged so CLI renderers are untouched. **Every lane opens its own target-anchored Catalog connection** — which FIXED a latent bug the parallelization forced into the open: the TUI's analysis workers (`_analyze_once`/`_finalize`) opened a fresh Catalog with NO active target, so fields/dictionary/findings silently landed in the reserved (unassigned) bucket (id=0). Session 23's on-device `glyph target show example.com` had proven it: findings 0 / fields 0 / dictionary 0 while the dashboard displayed them. Rewired `glyph/cli/run.py::_gather` + the TUI (ticks = schema+rosetta ∥ sensitive, no snihunt per ADR-10; finalize = all three) to delegate. **165 tests pass / 5 skip** (was 161/5; +4 new pipeline tests: a threading.Barrier test that PROVES the three lanes overlap in time, a target-anchoring regression test, and flag-skip tests). ADR-15 appended (accepted/implemented). Reviewer (code-reviewer-deepseek-flash) ran 4 parallel passes — clean; it caught the test seed's 1-letter TLD (`a@s.t` — email detector correctly ignores it → 0 findings; fixed to `a@shop.ke`) and a connection leak, plus one cosmetic nit applied (test lambda → named helpers).
- **Open items:** capture-tool parallelism (Playwright ∥ mitmproxy addon against one target — the OTHER reading of 'different tools') remains unbuilt; backlog's 'mitmproxy vs Playwright live head-to-head' points at it. Windows-box live-TUI verification (now optional). Rest of backlog unchanged.
- **Report:** .context/memory/reviews/2026-08-01-parallel-analysis-pipeline.md

## Session 25 (2026-08-01) — Buffy / deepseek-v4-flash — ADR-15 wall-clock profile
- **Agent:** Buffy | **Model:** deepseek/deepseek-v4-flash | **Platform:** bao@local macOS | **Role:** engineer | **Core:** 0.5.0
- **Task:** Target: 'Profile `glyph run har` before/after ADR-15 on a larger HAR to quantify the wall-clock speedup from parallel lanes (expected: max instead of sum).'
- **Commits:** none (product) — benchmark-only session; this `chore(context):` bookkeeping.
- **Outcome:** done — NO product code changed. Throwaway benchmarks (/tmp/prof_har.py + /tmp/prof_io.py): before = reconstructed old _gather order (infer_all→build_dictionary→run_scan→run_hunt on ONE connection), after = run_analysis() 3 lanes; same synthetic HAR (~340/700 flows), same flags, fresh DB per rep, ingest excluded, median 3–4 reps, parity check. Results: (1) OFFLINE net=False (CPU-bound): 1.00x (seq 2.412s / par 2.403s) — schema 54% + sensitive 36% are pure-Python CPU the GIL serializes; zero regression. (2) REAL NETWORK: jitter swamps (0.41x–1.44x) but the timeline proved the overlap (CPU lanes done at 2.9s under snihunt's ~31s I/O). (3) CONTROLLED I/O (fixed sleeps on CT/DNS/reverse-IP, 2.5x HAR): 1.44x deterministic (seq 17.63s / par 12.24s) — ~6s of CPU hides entirely under snihunt's ~12s I/O; parity identical (1068/54/574/14). Conclusion: ADR-15's win is I/O-bound-only — ~1.4x when snihunt does network recon, ~1.0x offline; max-instead-of-sum confirmed on the I/O path. Reviewer ran 2 passes on methodology + results (clean; flagged lane windows as upper bounds, rosetta underweighted on synthetic HAR, offline uses net=False both sides — all stated in the report).
- **Open items:** unchanged (capture-tool parallelism; Windows-box live-TUI optional).
- **Report:** .context/memory/reviews/2026-08-01-adr15-parallel-profile.md
## Session 26 (2026-08-01) — Buffy / deepseek-v4-flash — tables scoped to current target
- **Agent:** Buffy | **Model:** deepseek/deepseek-v4-flash | **Platform:** bao@local macOS | **Role:** engineer | **Core:** 0.5.0
- **Task:** Target (user report): 'Tables display. The outputs are not being filtered by session or target, it fetches all from db instead of displaying only those that are tied to the current target.'
- **Commits:** product commit (feat(catalog): persist + restore active target) + this `chore(context):` bookkeeping. See `git log`.
