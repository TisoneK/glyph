...
| ID | Summary |
|----|---------|
| B-2026-09-23-3 | Real-world browse verification on the user's primary browser (Brave) against an auth-protected target: log in, visit deposit/withdraw/send, detach, then confirm the catalog holds the auth+payment endpoints and `glyph sensitive` flags credentials/tokens/OTPs. |
| B-2026-09-23-4 | Add a bounded graceful-shutdown timeout/fallback so a pathological browser or network hang cannot leave capture workers or the TUI stuck. |

### Low Priority

| ID | Summary |
|----|---------|
| B-2026-09-23-5 | Add strict historical capture-session isolation for all-tabs browser capture (a schema feature: per-session tagging of flows captured in all-tabs mode). |
